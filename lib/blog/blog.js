...
Blog.save = function(article, callback) {
    Blog.getCollection(function(error, article_collection) {
        ...
        Blog.findBySlug(article.originalSlug, function(error, foundArticle) {
            ...
            savedArticle.allowComments = article.allowComments;
            savedArticle.showComments = article.showComments;
                ...
                article_collection.update(
                    {_id: foundArticle._id},
                    {
                        $set: {
                            ...
                            allowComments: savedArticle.allowComments,
                            showComments: savedArticle.showComments
                        }
                    },
                function(error, result) {
                    if (error) callback(error);
