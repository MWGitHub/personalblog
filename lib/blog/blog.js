    ...
    Blog.getCollection(function(error, article_collection) {
        if (error) callback(error)
            ...
            article_collection.findOne({
                _id: Database.createIdFromHex(id)
    ...
    Blog.getCollection(function(error, article_collection) {
        ...
        Blog.findById(article.id, function(error, foundArticle) {
            ...
            article_collection.update(
                {_id: article.id ? Database.createIdFromHex(article.id) : Database.createObjectId()},
                {
                    $set: {
                        slug: article.slug,
    ...
    Blog.getCollection(function(error, article_collection) {
        if (error) callback(error)
            ...
            article_collection.remove(
                {_id: Database.createIdFromHex(articleId)},
                true,
                function(error, article) {
                    if (error) callback(error);
                    else callback(null, article);
        ...
        if (article.allowComments) {
            Blog.getCollection(function(error, article_collection) {
                if (error) callback(error);
                    ...
                    article_collection.update(
                        {_id: aDatabase.createIdFromHex(articleId)},
